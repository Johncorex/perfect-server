...
void ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	if (player->hasCondition(CONDITION_EXHAUST, 1)) {
		player->sendTextMessage(MESSAGE_STATUS_SMALL, "Voce nao pode relogar muito rapido cara de pitomba.");
		return;
	}
	
	if (Condition* conditionlogout = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST, 4000, 0, false, 1)) {
		player->addCondition(conditionlogout);
    }
