...
void ProtocolGameBase::sendResourceData(ResourceType_t resourceType, int64_t amount)
{
	NetworkMessage msg;
	msg.addByte(0xEE);
...
void ProtocolGameBase::sendRefreshImbuingDialog(Item* item)
{
	...
	int16_t slots;
	g_imbuements.getItemSlotCount(item->getID(), slots);

	msg.addByte(slots);

	auto addImbuementData = [&](Imbuement* ib) {		
		msg.add<uint32_t>(ib->getId());
		msg.addString(ib->getName());
		msg.addString(ib->getDescription());
		msg.addString(ib->getGroup());

		//msg.add<uint16_t>(item->getClientID());
		msg.add<uint16_t>(ib->imageId);
		msg.add<uint32_t>(ib->getDuration());

		msg.addByte(0x00); // premium only

		const auto& sources = ib->getAstralSources();
		msg.addByte(sources.size());
		for (const auto& source : sources) {
			const ItemType& it = Item::items[source.first];
			msg.add<uint16_t>(it.clientId);
			msg.addString(it.name);
			msg.add<uint16_t>(source.second);
		}

		msg.add<uint32_t>(ib->getCost());
		msg.addByte(ib->getSuccessRate());
		msg.add<uint32_t>(ib->getProtectionCost());
	};

	for (int32_t offset = 0; offset < slots; offset++) {
		...
		if (duration) {
			...
			Imbuement* ib = g_imbuements.getImbuement(id);
			addImbuementData(ib);
			msg.add<uint32_t>(duration);
			msg.add<uint32_t>(ib->getRemovalCost());
		} else {
			msg.addByte(0x00);
		}
	}
	...
	for (Imbuement* ib : imbuements) {
		...
		const auto& sources = ib->getAstralSources();
		for (const auto& source : sources) {
			if (!astralSources.count(source.first)) {
				astralSources[source.first] = player->getItemTypeCount(source.first);
			}
		}
	}

	msg.add<uint32_t>(astralSources.size());

	for (const auto& source : astralSources) {
		const ItemType& it = Item::items[source.first];
		msg.add<uint16_t>(it.clientId);
		msg.add<uint16_t>(source.second);
	}
